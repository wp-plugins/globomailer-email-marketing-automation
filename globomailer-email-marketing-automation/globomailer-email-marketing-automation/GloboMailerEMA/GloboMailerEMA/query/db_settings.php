...
class gmema_cls_settings
{
	...
	public static function gmema_setting_update($data = array())
	{
		...
		$sSql = $wpdb->prepare("UPDATE `".$prefix."gmema_pluginconfig` SET 
			`gmema_c_list` = %s,`selected_fields` = %s,`transact` = %s,`transact_msg` = %s 
			WHERE gmema_c_id = %d	LIMIT 1", 
			array($data["gmema_c_list"],$data["selected_fields"],$data["transact"],$data["transact_msg"],$data["gmema_c_id"]));
		$wpdb->query($sSql);
		return "sus";
	}
	
	public static function gmema_home_update($data = array())
	{
		global $wpdb;
		$prefix = $wpdb->prefix;
		$sSql = $wpdb->prepare("UPDATE `".$prefix."gmema_pluginconfig` SET 
			`gmema_c_publickey` = %s,`gmema_c_privatekey` = %s
			WHERE gmema_c_id = %d	LIMIT 1", 
			array($data["gmema_c_publickey"],$data["gmema_c_privatekey"],$data["gmema_c_id"]));
		$wpdb->query($sSql);
		return "sus";
	}
	...
	{	
		if (empty($instance['gmema_c_list'])) {
	        return;
	    }
